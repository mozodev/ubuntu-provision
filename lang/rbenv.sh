...
su - $UBUNTU_USER -c "cd /home/$UBUNTU_USER/.rbenv && src/configure && make -C src"
...
if [ ! -e .rbenv/versions/$RUBY_VERSION ]; then
  su - $UBUNTU_USER -c "rbenv install $RUBY_VERSION"
  su - $UBUNTU_USER -c "rbenv global $RUBY_VERSION"
  su - $UBUNTU_USER -c "echo 'gem: --no-ri --no-rdoc --no-document --suggestions' >> ~/.gemrc"
  su - $UBUNTU_USER -c "gem install bundler"
fi
