...
allowed_apps=(hugo jq yq drupalorg benthos drush-launcher cloudflared notify_slack)
...
case "$1" in
  ...
  "notify_slack") gh_repo="catatsuy/notify_slack"; pattern="browser_download_url.*notify_slack-linux-amd64" ;;
  *) gh_repo=""; pattern="" ;;
esac
...
if [ ! -v $filename ] && [ -f $filename ]; then
  ...
  if [ "hugo" == "$1" ] || [ "drush-launcher" == "$1" ]; then
    $1 version
  elif [ "notify_slack" == "$1" ]; then
    $1 -version
  elif [ "benthos" == "$1" ]; then
    $1 -v
  else
